...
int N, M, color, bpp, linesize, datasize;
...
void fillheader(char header[]) {
    ...
    bih.biPlanes = 1;                           //
    bih.biBitCount = bpp*8;                     // число бит на пиксель
...
int main(char argc, char* argv[]) {
	...
	bpp = color ? 3 : 1;
	linesize = ceil(bpp * N / 4.0) * 4;

	datasize = M * linesize;     // !!!!!!! размер пиксельных данных, чтобы размер строки был кратен 4 байтам
